...
public class MyActivityManager {
	...
	public MyActivityManager(Context context) {
		try {
			Class<?> activityManagerClass = Class
					.forName("android.app.ActivityManager");
			mActivityManager = (ActivityManager) context
					.getSystemService(Context.ACTIVITY_SERVICE);

			mRemoveTask = activityManagerClass.getMethod("removeTask",
	...
	/**
	 * If set, the process of the root activity of the task will be killed as
	 * part of removing the task.
	 */
	...
	public boolean removeTask(int taskId, int flags) {
		Log.v("jajaja", "removing task");
		try {
			return (Boolean) mRemoveTask.invoke(mActivityManager,
					Integer.valueOf(taskId), Integer.valueOf(flags));
		} catch (Exception ex) {
	...
	public void clearRecentTasks() {
		List<RecentTaskInfo> recents = mActivityManager.getRecentTasks(1000,
